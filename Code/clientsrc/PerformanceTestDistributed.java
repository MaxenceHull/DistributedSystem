...
public class PerformanceTestDistributed {
    ...
    public static void main(String args[]) {
        ...
        if(args.length > 2){
            load = Double.parseDouble(args[2]);
        }
        if (args.length > 3) {
        ...
        while (true){
            ...
            //}
            System.out.println(sum/inc);
            long sleep = Math.round((1e3/ load) - timestamp);
            if(sleep > 0){
                try {
                    if(inc % 1000 == 0){
                        System.out.println(sum/inc);
    ...
    private static void transaction_4() throws TransactionAbortedException, RemoteException, InvalidTransactionException {
        ...
        for(int i=0; i<3; i++){
            if(choice == 0){
                int indexFlight = rand.nextInt(flights.size());
                addAFlight(idTransaction, flights.get(indexFlight), 1, 1000);
            }else if(choice == 1){
                int indexRoom = rand.nextInt(rooms.size());
                addARoom(idTransaction, rooms.get(indexRoom), 1,3444);
            } else if(choice == 2){
                int indexCar = rand.nextInt(cars.size());
                addACar(idTransaction, cars.get(indexCar), 1, 785);
            }
        }
